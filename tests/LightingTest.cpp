...
TestScene::TestScene(Window& window) :
		...
		vml(*this), vfblr(*this, vp::VFBLR::WSADSC, 8)
{
	...
	sun = std::make_shared<zg::entities::NDParametricCurve<3>>(
		window, *this, dl.position, glm::vec3(0), glm::vec3(4.3),
		glm::vec4(0.944, 1.00, 0.760, 1),
		shaders::RuntimeConstants(),
		commonLineThickness, 0.0f, 2.0f * PI, commonDt,
	...
	for (char j = 0; j < 3; j++)
	{
		...
		if (rand > 29)
		{
			...
			auto simpleHelix = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				commonShaderConstants, commonLineThickness, 0.0f, 4.0f * PI, commonDt, "2*cos(t)", "2*sin(t)", "0.5*t");
		...
		else if (rand > 28)
		{
			...
			auto conicalHelix = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 8.0f * PI, PI / 128.f, // Smaller dt for larger range
		...
		else if (rand > 27)
		{
			...
			auto toroidalSpiral1 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 2.0f * PI, commonDt, "(5 + 1.5 * cos(5 * t)) * cos(t)",
				"(5 + 1.5 * cos(5 * t)) * sin(t)", "1.5 * sin(5 * t)");
		...
		else if (rand > 26)
		{
			...
			auto trefoilKnot = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "sin(t) + 2 * sin(2 * t)", "cos(t) - 2 * cos(2 * t)",
				"-sin(3 * t)");
		...
		else if (rand > 25)
		{
			...
			auto sphericalSpiral = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.01f, PI - 0.01f, PI / 128.f, // Avoid poles t=0, t=PI if parser struggles
		...
		else if (rand > 24)
		{
			...
			auto viviani = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 4.0f * PI, commonDt, "1 * (1 + cos(t))", "1 * sin(t)", "2 * sin(t / 2)");
			addEntity(viviani);
		}
		else if (rand > 23)
		{
			...
			auto lissajous3D_123 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "sin(t)", "sin(2 * t + acos(-1) / 3)", "cos(3 * t)");
		...
		else if (rand > 22)
		{
			...
			auto butterfly3D = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				commonLineThickness, 0.0f, 12.0f * PI, PI / 256.f, // Needs high resolution & range
		...
		else if (rand > 21)
		{
			...
			auto figureEightKnot = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "(2 + cos(2 * t)) * cos(3 * t)",
		...
		else if (rand > 20)
		{
			...
			auto ellipticalHelix = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 6.0f * PI, commonDt, "4 * cos(t)", "2 * sin(t)",
		...
		else if (rand > 19)
		{
			...
			auto toroidalSpiral2 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 2.0f * PI, PI / 128.f, // Needs more resolution
		...
		else if (rand > 18)
		{
			...
			auto archimedeanRaised = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 10.0f * PI, PI / 128.f, "0.2 * t * cos(t)",
				"0.2 * t * sin(t)",
		...
		else if (rand > 17)
		{
			...
			auto lemniscateGerono3D = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "sin(t)", "sin(t) * cos(t)",
		...
		else if (rand > 16)
		{
			...
			auto helixOnCone = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.1f, 10.0f, 0.05f, // Using linear t range for simplicity
		...
		else if (rand > 15)
		{
			...
			auto astroid3D = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "pow(cos(t), 3)", "pow(sin(t), 3)",
		...
		else if (rand > 14)
		{
			...
			auto cinquefoilKnot = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "(2 + cos(5 * t / 2)) * cos(t)",
		...
		else if (rand > 13)
		{
			...
			auto twistedCubic = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, -5.0f, 5.0f,
		...
		else if (rand > 12)
		{
			...
			auto lissajous3D_321 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "cos(3 * t)", "cos(2 * t + acos(-1) / 2)", "sin(t)");
		...
		else if (rand > 11)
		{
			...
			auto dampedHelix = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 10.0f * PI, PI / 128.f,
		...
		else if (rand > 10)
		{
			...
			auto saddleCurve = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, -2.0f * PI, 2.0f * PI, commonDt,
		...
		else if (rand > 9)
		{
			...
			auto billsCurve = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 12.0f * PI, PI / 128.f, // Needs range
		...
		else if (rand > 8)
		{
			...
			auto paraboloidSpiral = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 4.0f * PI, PI / 128.f,
		...
		else if (rand > 7)
		{
			...
			auto dennisCurve = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 10.0f * PI, PI / 128.f,
		...
		else if (rand > 6)
		{
			...
			auto torusKnot23 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "(2 + cos(3 * t)) * cos(2 * t)",
		...
		else if (rand > 5)
		{
			...
			auto twistedHeart3D = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				commonShaderConstants, commonLineThickness, 0.0f, 2.0f * PI, commonDt, "16 * pow(sin(t), 3)",
		...
		else if (rand > 3)
		{
			...
			auto logSpiralRaised = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 6.0f * PI, PI / 128.f,
		...
		else if (rand > 2)
		{
			...
			auto hypotrochoid3D = std::make_shared<zg::entities::NDParametricCurve<3>>(
				window, *this, calculatePosition(j), glm::vec3(0), glm::vec3(0.5),
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, commonLineThickness, 0.0f, 10.0f * PI, PI / 128.f, // Needs range for inner loops
		...
		else if (rand > 1)
		{
			...
			auto lissajous3D_456 = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, PI / 128.f, // More resolution needed
		...
		else if (rand > 0)
		{
			...
			auto mobiusPath = std::make_shared<zg::entities::NDParametricCurve<3>>(
				...
				glm::vec4(Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), Random::value<float>(0.2, 0.9), 1),
				commonShaderConstants, // Scaled up
				commonLineThickness, 0.0f, 2.0f * PI, commonDt, "(1 + 0.5 * cos(t / 2.0)) * cos(t)",
	...
	cubeEntity = std::make_shared<entities::Cube>((Window&)window, *this, centerPos + glm::vec3(0, 2, 0), // position
	...
	addEntity(std::make_shared<entities::Cube>((Window&)window, *this, centerPos, glm::vec3(0, 0, 0), glm::vec3(1, 1, 1),
																						 glm::vec3(16 * 8, 1.3, 32 * 8),
																						 shaders::RuntimeConstants({"Lighting", "DirectionalLightShadowMaps",
																																				// "SpotLightShadowMaps",
																																				// "PointLightShadowMaps",
																																				"LightSpacePosition", "Fog"})));
	// auto &shader = cubeEntity->addedShader();
