...
namespace zg::audio
{
	...
	private:
		std::map<size_t, LayeredWave> layeredWaves;
		...
		double envelopeDuration = 0.05; // Duration of fade in/out in seconds (e.g., 10ms)

	public:
		// Constructor
		Oscillator(AudioEngine& _audioEngine, double amp = 0.5);
		size_t addWaveLayer(WaveGeneratorFunc func, const WaveParameters& params);
		bool removeWaveLayer(size_t& ID);
