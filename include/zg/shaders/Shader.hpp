...
namespace zg::shaders
{
	struct Shader
	{
		...
		int32_t getSSBO_BindingIndex(const std::string_view name);
		int32_t getUBO_BindingIndex(const std::string_view name);
		...
		void setUniform(const std::string_view name, vaos::VAO &vao, const T &value, uint32_t size = 0)
		{
			...
			iRenderer->setUniform(*this, vao, name, &value, pointerSize, uniformType);
		};
		...
		{
			auto pointerSize = size ? size : sizeof(value);
			iRenderer->setBlock(*this, vao, name, &value, pointerSize);
		};
