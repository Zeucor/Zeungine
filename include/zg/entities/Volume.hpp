...
namespace zg::entities
{
	...
	public:
		...
		NUVVolume(zg::Window& window_in, zg::Scene& scene_in, glm::vec3 position, glm::quat rotation, glm::vec3 scale,
							...
							double vStart_in, double vEnd_in, double vStep_in, const std::string& t_equation, const Args&... args) :
				Entity(window_in, scene_in,
							 ...
							 getElementCount(uStart_in, uEnd_in, uStep_in, vStart_in, vEnd_in, vStep_in), {}, position, rotation,
		...
		size_t getElementCount(double u_start, double u_end, double u_step, double v_start, double v_end,
													 double v_step) const
		{
			size_t u, v;
		...
		void generateMeshAndNormals()
		{
			...
			std::vector<glm::vec3> calculated_positions;
			std::vector<glm::vec3> calculated_normals;
			calculated_positions.reserve(expectedCount);
            ...
            for (size_t j = 0; j < vCount; ++j)
            {
                ...
                for (size_t i = 0; i < uCount; ++i)
                {
                    ...
                    if (point_is_valid)
                    {
                        ...
                        calculated_positions.push_back(current_pos);
                    }
                    else
                    {
                        calculated_positions.push_back(current_pos);
                    ...
                    if (useAnalyticalNormals)
                    {
                        ...
                        {
                            calculated_normals[vertex_index] = glm::vec3(0.0f, 0.0f, 1.0f);
                            std::cerr << "Warning: NUVVolume - Analytical normal is zero vector at u=" << u << ", v=" << v
            ...
            if (!useAnalyticalNormals)
            {
                computeNormals(frontFace, calculated_indices, calculated_positions, calculated_normals);
            }

			size_t actualVertexCount = calculated_positions.size();
			...
			// }
			updateGpuData(calculated_positions, calculated_normals, calculated_indices, calculated_colors);
		}
		...
		void updateGpuData(const std::vector<glm::vec3>& new_positions, const std::vector<glm::vec3>& new_normals,
											 const std::vector<uint32_t>& new_indices, const std::vector<glm::vec4>& new_colors)
		{
			...
			updateElements("Position", this->positions);
		...
		bool preRender() override
		{
			...
			shader->setBlock("CameraPosition", *this, scene.viewPointer ? scene.viewPointer->position : glm::vec3(0.0f), 16);
			shader->unbind();
			return true;
		}
		...
		void setColor(const glm::vec4& new_color)
		{
			...
			if (this->colors.size() != this->positions.size())
			{
				this->colors.resize(this->positions.size());
			}
