...
namespace anex::modules::gl
{
	...
	struct GLWindow : IWindow
	{
		...
		float dpiScale = 1.0f;
		GLWindow(const char *title, const int32_t &windowWidth, const int32_t &windowHeight, const int32_t &windowX, const int32_t &windowY, const bool &borderless = false, const uint32_t &framerate = 60);
		GLWindow(GLWindow &parentWindow, const char *childTitle, const int32_t &childWindowWidth, const int32_t &childWindowHeight, const int32_t &childWindowX, const int32_t &childWindowY, const uint32_t &framerate = 60);
		~GLWindow();
