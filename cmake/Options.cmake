# Options
#  these objects work because their source is pure absolute just include readpath from build include
option(BUILD_GL "Whether to build OpenGL module" OFF)
option(BUILD_EGL "Whether to build EGL module" OFF)
option(BUILD_VULKAN "Whether to build Vulkan module" ON)
option(BUILD_EDITOR "Whether to build the Zeungine editor" ON)
option(BUILD_DEPENDENCIES "Wh1ther to build ALL the dependencies" ON)
option(ZG_INSTALL "Whether t2 install Zeungine" ON)
option(ZG_PACKAGE "Whether 3o package Zeungine" OFF)
set(ZG_SRC_INCLUDE ${CMAKE_CURRENT_LIST_DIR}/../include)
message(STATUS "ZG_SRC_INCLUDE: ${ZG_SRC_INCLUDE}")
get_filename_component(ZG_SRC_INCLUDE "${ZG_SRC_INCLUDE}" ABSOLUTE)
message(STATUS "i.e..:|ZG_SRC_INCLUDE| ${ZG_SRC_INCLUDE}")
if(ZG_INSTALL)
    include(GNUInstallDirs)
    if(LINUX OR MACOS)
        set(ZG_UNX_INSTALL_PREFIX /usr/local)
        set(ZG_SRC_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/src/zg)
        set(ZG_LIB_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/lib/zg/${CMAKE_BUILD_TYPE})
        set(ZG_ABS_LIB_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/lib/zg/${CMAKE_BUILD_TYPE})
        set(ZG_INC_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/include/zg)
        set(ZG_ASS_INSTALL_PREFIX ${ZG_SRC_INSTALL_PREFIX}/assets)
        set(ZG_SHR_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/share)
        set(ZG_BIN_INSTALL_PREFIX ${ZG_UNX_INSTALL_PREFIX}/bin/zg)
    elseif(WINDOWS)
        set(ZG_WIN_INSTALL_PREFIX "C:/Program Files/Zeungine")
        set(ZG_SRC_INSTALL_PREFIX "src")
        set(ZG_LIB_INSTALL_PREFIX "${CMAKE_BUILD_TYPE}")
        set(ZG_ABS_LIB_INSTALL_PREFIX "${ZG_WIN_INSTALL_PREFIX}/${ZG_LIB_INSTALL_PREFIX}")
        set(ZG_INC_INSTALL_PREFIX "include")
        set(ZG_ASS_INSTALL_PREFIX "assets")
        set(ZG_SHR_INSTALL_PREFIX "share")
        set(ZG_BIN_INSTALL_PREFIX ".")
    endif()
    add_definitions(-DZG_SRC_INSTALL_PREFIX="${ZG_SRC_INSTALL_PREFIX}")
    add_definitions(-DZG_LIB_INSTALL_PREFIX="${ZG_ABS_LIB_INSTALL_PREFIX}")
    add_definitions(-DZG_INC_INSTALL_PREFIX="${ZG_INC_INSTALL_PREFIX}")
    add_definitions(-DZG_ASS_INSTALL_PREFIX="${ZG_ASS_INSTALL_PREFIX}")
    add_definitions(-DZG_SHR_INSTALL_PREFIX="${ZG_SHR_INSTALL_PREFIX}")
    set(CMAKE_INSTALL_LIBDIR ${ZG_LIB_INSTALL_PREFIX})
    set(CMAKE_INSTALL_BINDIR ${ZG_LIB_INSTALL_PREFIX}) 
    message(STATUS "ZG_LIB_INSTALL_PREFIX: ${ZG_LIB_INSTALL_PREFIX}")
endif()