...
Frame::Frame(zg::Window& window, zg::Scene& scene, glm::vec3 position, glm::quat rotation, glm::vec3 scale,
		...
		zg::Entity(window, scene,
							 ...
							 getIndiceCount(), getIndices(window), getElementCount(), getElements(size, borderWidth), position,
...
uint32_t Frame::getElementCount() { return 8; }
...
{
	this->size = size;
	positions = getElements(this->size, borderWidth);
	updateElements("Position", positions);
};
