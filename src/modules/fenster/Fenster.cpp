...
FensterWindow::FensterWindow(const char* title, const uint32_t &windowWidth, const uint32_t &windowHeight, const int32_t &windowX, const int32_t &windowY, const bool &borderless, const uint32_t &framerate):
...
void anex::modules::fenster::fenster_rect(struct fenster* f, int x, int y, int w, int h,
                  uint32_t c)
{
...
void anex::modules::fenster::fenster_circle(struct fenster* f, int x, int y, int r, uint32_t c)
{
...
void anex::modules::fenster::fenster_fill(struct fenster* f, int x, int y, uint32_t old,
                  uint32_t c)
{
...
// clang-format on
void anex::modules::fenster::fenster_text(struct fenster* f, int x, int y, const char* s, int scale,
                  uint32_t c)
{
  while (*s)
  {
...
std::pair<int, int> anex::modules::fenster::fenster_text_bounds(const char* s, int scale)
{
  int x = 0, y = 5 * scale;
  while (*s)
  {
