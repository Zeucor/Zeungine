#include <zg/Registry.hpp>
#include <string>
zg::Registry::WindowKeyIDVector zg::Registry::windows([](auto& window) { return window.title; });
/**
 * Gets a Window from the registry by the Window ID
 */
zg::Window& zg::Registry::getWindow(const std::vector<size_t>& idStack)
{
    auto idStackData = idStack.data();
    if (idStack.size() < 1)
        throw std::runtime_error("Cannot find a Scene with a stack size of less than 1");
    auto iter = windows.find_id(idStackData[0]);
    if (iter == windows.end())
        throw std::runtime_error("Window not found with idStack[0] = " + std::to_string(idStackData[0]));
    return *iter;
}
/**
 * Emplaces a Window at the back of the Windows' key/id vector, returning the tuple containing key/id/*index/*value
 * 
 * *value can become degenerate if adding multiple windows/scenes/entities, so use ID (stacks) instead
 */
zg::Registry::WindowKeyIDVector::EmplaceBackTuple zg::Registry::addWindow(const WindowCreateInfo& createInfo)
{
    auto window_tuple = windows.emplace_back(createInfo);
    auto& windowID = std::get<KEY_ID_VECTOR_ID_INDEX>(window_tuple);
    auto& window = *std::get<KEY_ID_VECTOR_VALUE_INDEX>(window_tuple);
    window.ID = windowID;
    return window_tuple;
}
/**
 * Gets a Scene from the registry by looking
 *   up window @ idStack[0],
 *   then window.scenes[idStack[1]]
 */
zg::Scene& zg::Registry::getScene(const std::vector<size_t>& idStack)
{
    auto idStackData = idStack.data();
    if (idStack.size() < 2)
        throw std::runtime_error("Cannot find a Scene with a stack size of less than 2");
    auto& window = getWindow(idStack);
    auto sceneIter = window.scenes.find_id(idStackData[1]);
    if (sceneIter == window.scenes.end())
        throw std::runtime_error("Scene not found with idStack[1] = " + std::to_string(idStackData[1]));
    return *sceneIter;
}
/**
 * Gets an Entity from the registry by looking up..
 *   window @ idStack[0],
 *   then window.scenes[idStack[1]],
 *   then scenes.entities[idStack[3]] = &Entity
 *   of (if needbe) [...entity.children[idStack[4..n-1]]] = &Entity
 */
zg::Entity& zg::Registry::getEntity(const std::vector<size_t>& idStack)
{
    auto& scene = getScene(idStack);
    if (idStack.size() < 3)
        throw std::runtime_error("Cannot find an Entity with a stack size of less than 2");
    auto idStackData = idStack.data();
    auto entityIter = scene.entities.find_id(idStackData[2]);
    if (entityIter == scene.entities.end())
        throw std::runtime_error("Entity not found with idStack[2] = " + std::to_string(idStackData[2]));
    auto& entity = *entityIter;
    auto currentEntity = &entity;
    auto idStackSize = idStack.size();
    for (int i = 3; i < idStackSize; i++)
    {
        auto& currentEntityRef = *currentEntity;
        auto childIter = currentEntityRef.children.find_id(idStackData[i]);
        if (childIter == currentEntityRef.children.end())
            throw std::runtime_error("Child Entity not found with idStack[" + std::to_string(i) + "] = " + std::to_string(idStackData[i]));
        currentEntity = &*childIter;
    }
    return *currentEntity;
}